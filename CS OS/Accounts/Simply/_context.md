# Simply

**Last Updated:** 2026-06-10
**Updated By:** CSA OS (external channel scan)
**Why Updated:** Jun 9 — customer asking about iOS 26 Session Replay privacy issue timeline (unreplied)

---

## Identity

| Field | Value |
|-------|-------|
| ARR | $169,466 |
| Renewal | 2027-02-28 |
| Industry | Media & Entertainment |
| Segment | SMB |
| Region | EMEA |
| Website | hellosimply.com |
| AE | ShavitBen-Itzhak (shavit.ben-itzhak@mixpanel.com) |
| Sales Manager | Ofer Polivoda |
| SFDC | [0013q00002L1gUCAAZ](https://mixpanel.lightning.force.com/lightning/r/Account/0013q00002L1gUCAAZ/view) |
| GDoc | [Account Plan](https://docs.google.com/document/d/1dy8HiiBoqRYGPAtZE2DVX49AMJ1xZOWI1-VaAizxVik/edit?usp=drivesdk) |

---

## Key Contacts

_No contacts data available._

---

## Current Health

| Metric | Value |
|--------|-------|
| Overall | High |
| VHS Score | 5.0 |
| Adoption Score | 5.0 |
| Sentiment | Mixed |
| Renewal Risk | Low |

Healthy and active. The customer is actively exploring new features like Session Replay UI controls and preparing for an early renewal.

---

## Executive Summary

As of 2026-05-31, Simply is highly engaged with an active pipeline deal 'Simply - FY 28 Early Renewal + Upsell' in Stage 1: Pipeline for $190,000 ARR. Last interaction: 2026-05-31 via Slack Message. Total Calls: 1. Total Emails: 7. A recent call on May 19 with Roman Brisker covered Session Replay UI controls, and the team is preparing for a Quarterly Meeting on June 3. The company recently rebranded to Simply and reached $200M ARR ([GetLatka](https://getlatka.com/companies/joytunes)).

---

## Active Risks

- **[ACTIVE]** [2026-06-03] Budget constraints for paid UI controls feature
- **[ACTIVE]** [2026-06-10] Jun 9: iOS 26+ (Liquid Glass) Session Replay privacy issue — Yonah replied Jun 9, asked for SDK version. Customer confirmed v1.5.0 + mixpanel-swift 6.4.0. Yonah asked if they're seeing the issue in replays (Jun 10). Active investigation ongoing.


---

## Open Items

- [ ] **[DUE 2026-06-03]** Conduct Quarterly Meeting and advance early renewal (Owner: ShavitBen-Itzhak) — pending
- [x] **[DUE Completed]** Schedule team walkthrough for Session Replay UI controls — done, per Yonah DM note Jul 16 ("already walked through the UI controls"). (Owner: Yonah Dorfman)
- [ ] **[DUE 2026-06-11]** Close out iOS 26 SR investigation — SDK v1.5.0 confirmed, asked if they're seeing the issue in replays. Determine if this is a confirmed bug and get product/eng timeline (Owner: Yonah Dorfman) — in progress


---

## Sales Intelligence

| Field | Value |
|-------|-------|
| Latest Opp | Simply - FY 28 Early Renewal + Upsell |
| Stage | 1: Pipeline |
| Opp ARR | 190000.0 |

---

## Recent History

**2026-07-21 | Slack full catch-up — SR walkthrough closed out, new open technical thread on local_time property**
- The long-overdue Session Replay UI-controls team walkthrough with Roman's team finally happened (closed an action item open since May 27).
- New unresolved technical thread in #ext-mixpanel-simply (opened Jul 12, still active as of Jul 20, 20 replies): customer wants `local_time` property to reflect user's local time rather than project timezone — needs a resolution/workaround.
- FY28 Early Renewal + Upsell ($190K) remains at Stage 1: Pipeline with no commercial movement this cycle.
- Usage and Session Replay adoption both healthy and growing (70 replays watched last week).

**2026-07-16 | DM note [via Slack DM]**
- Already walked through the UI controls.

**2026-06-10 | External channel scan [Jun 9 message — IN PROGRESS]**
- Customer asked for a timeline on iOS 26 (Liquid Glass) SR privacy issue. Yonah replied Jun 9, asked for SDK version. Customer confirmed v1.5.0 + mixpanel-swift 6.4.0. Yonah is actively investigating as of Jun 10.
- Product gap could stall the $190K renewal/upsell conversation if unresolved — get engineering timeline before the renewal discussion.
