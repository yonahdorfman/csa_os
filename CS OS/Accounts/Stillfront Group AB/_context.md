# Stillfront Group AB

**Last Updated:** 2026-07-15
**Updated By:** CSA OS (morning cycle — Gong harvest)
**Why Updated:** New risk surfaced — Playa Games (a Stillfront studio) has signaled non-renewal intent on its Mixpanel pilot.
**Previous:** 2026-06-10 — external channel scan (warehouse sync 403 Forbidden, unresolved).

---

## Machine Patch — 2026-07-15 (Morning Cycle)
### 🔴 New risk: Playa Games (Stillfront studio) non-renewal intent
- **2026-07-01 Gong call (23 min):** Alissa (Mixpanel) with Suman Majety (Stillfront). Stillfront runs a decentralized "studio-first" model — 15 studios, ~60 games, 1,200 people, HQ Stockholm. Mixpanel was piloted with one studio, Playa Games, as a trial for possible wider studio adoption.
- Playa Games has signaled **non-renewal intent** despite high usage — the specific game using Mixpanel is underperforming, investment is being scaled back, and a Mixpanel price increase compounded the decision. Alissa will contact Torsten (Playa's studio head) directly, with Suman cc'd.
- Positive counter-signal: Suman is personally positive on Mixpanel, interested in the new AI agent/headless features, and open to revisiting integration for growing games in Q4. Suman also invited to a Mixpanel AI event in Stockholm.
- Stillfront is building a centralized SDK to let studios pick pre-approved vendors — worth tracking as a potential wider-adoption channel if Mixpanel gets on the approved list.
- Action: follow up with Alissa on the direct Torsten conversation outcome before writing this off as a lost studio — this is a single-studio risk within a much larger group account, not a Stillfront-wide churn signal.
- Source: Gong call (call_id 4077546260569942982), harvested via morning-cycle 2026-07-15.

---

## Identity

| Field | Value |
|-------|-------|
| ARR | $12,348 |
| Renewal | — |
| Industry | Media & Entertainment |
| Segment | ENT |
| Region | EMEA |
| Website | www.stillfront.com |
| AE | AlissaVaughn (alissa.vaughn@mixpanel.com) |
| Sales Manager | Rocio Elizalde |
| SFDC | [0010Z000024meJAQAY](https://mixpanel.lightning.force.com/lightning/r/Account/0010Z000024meJAQAY/view) |
| GDoc | [Account Plan](—) |

---

## Key Contacts

| Name | Title | Email | Interactions |
|------|-------|-------|-------------|
| Nils Lobe | — | nlobe@playa-games.com | 3 |
| Alexander Padberg | — | apadberg@playa-games.com | 1 |
| Suman Majety | — | — | 1 |

---

## Current Health

| Metric | Value |
|--------|-------|
| Overall | — |
| VHS Score | 10.0 |
| Adoption Score | 10.0 |
| Sentiment | — |
| Renewal Risk | — |

_No health assessment available._

---

## Executive Summary

_No executive summary available._

---

## Active Risks

- **[ACTIVE]** [2026-06-10] Warehouse import 403 Forbidden (Ben Girard deactivated). Yonah replied May 11. Gidi Eigenstein (ex-AM) clarified: warehouse import was **never in the contract** — Gidi had been manually enabling it for free every few months. Billing team re-open requested; AE Alexander Padberg + Suman Majety tagged. Immediate access should be restored, but contractual status needs resolution with new AM.


---

## Open Items

- [ ] **[OVERDUE]** Confirm warehouse access restored + contractual status with AE Alexander Padberg (Owner: Yonah Dorfman)
- [ ] **[OVERDUE since Apr 6]** Send usage summary to Nils Lobe — he DM'd requesting it Apr 6; still unanswered as of Apr 26

---

## Sales Intelligence

| Field | Value |
|-------|-------|
| Latest Opp | — |
| Stage | — |
| Opp ARR | — |

---

## Recent History

**2026-06-10 | External channel scan [May 11 message — PARTIALLY RESOLVED]**
- Warehouse 403 Forbidden (Ben Girard deactivated). Yonah replied May 11: "Hi! Let me look into that for you."
- Gidi Eigenstein (ex-AM) stepped in and revealed: warehouse import was never in contract — he'd been manually enabling it for free every few months. He submitted billing request to re-open it and tagged AE Alexander Padberg + Suman Majety.
- Customer: "thanks for help" — immediate issue resolved. However, warehouse import being off-contract is a risk and expansion signal. Need AE to formalize or flag for upsell.
- Coaching note: "30 days unreplied" was incorrect — Yonah replied same day. Gidi handled the substance.

**2026-06-11 | DM note [via Slack DM]**
- "Still front is resolved and not an issue" — Yonah confirmed the Stillfront warehouse issue is resolved and no longer a concern.

**2026-04-26 | Weekly summary [CSA OS]**
- Usage dropped 72% WoW to 414 queries from only 5 active users — all Playa Games team (nlobe, skolodzey, apadberg)
- Nils Lobe requested usage summary via DM Apr 6 — still unanswered as of Apr 26
- No Slack or email signals since Apr 19
